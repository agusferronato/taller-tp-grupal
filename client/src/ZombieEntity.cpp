...
void ZombieEntity::render(SDL2pp::Renderer &renderer, Camera &camera, unsigned int it)
{
    unsigned int animationIt = zombie.getIsMoving() ? it : 0;

    Sprite src = textureManager.getZombieSprite(ZombieEntity::BodyTextureID, zombie.getDirection(), animationIt);
...
int ZombieEntity::get_x()
{
    return zombie.get_x();
}

int ZombieEntity::get_y()
{
    return zombie.get_y();
}

int ZombieEntity::get_h()
{
    return zombie.get_h();
}
