...
Sprite TextureManager::getBodySprite(int bodyId, Direction dir, unsigned int it) {
    auto& layout = std::get<BodyLayout>(texturesFrames.at(TextureLayoutType::Body));
...
Sprite TextureManager::getHeadSprite(int headId, Direction dir) {
    auto& layout = std::get<HeadLayout>(texturesFrames.at(TextureLayoutType::Head));
