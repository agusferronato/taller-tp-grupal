...
BackgroundPage::BackgroundPage(QWidget *parent) : QWidget(parent) {
  ...
  if (!fog.isNull()) {
    scaledFog = fog.scaled(width() * 2, height(), Qt::IgnoreAspectRatio,
                           Qt::SmoothTransformation);
  }
  if (!background.isNull()) {
    scaledBackground = background.scaled(size(), Qt::KeepAspectRatioByExpanding,
                                         Qt::SmoothTransformation);
  }
...
void BackgroundPage::paintEvent(QPaintEvent * /*event*/) {
  ...
  if (backgroundDarkness > 0.0) {
    ...
    painter.fillRect(
        rect(), QColor(0, 0, 0, static_cast<int>(backgroundDarkness * 255)));
  }
}
...
QFrame *BackgroundPage::createContentPanel() {
  ...
  panel->setStyleSheet("#contentPanel {"
