...
void PlayerEntity::render(SDL2pp::Renderer &renderer, Camera &camera, unsigned int it) {
...
void PlayerEntity::renderBody(SDL2pp::Renderer &renderer, Camera &camera, unsigned int it) {
    ...
    SDL2pp::Rect dst = camera.toScreen(player.get_x(), player.get_y(), Player::Width, Player::Height);
...
void PlayerEntity::renderName(SDL2pp::Renderer &renderer, Camera &camera) {
    ...
    SDL2pp::Rect playerPosition = camera.toScreen(player.get_x(), player.get_y(), Player::Width, Player::Height);
...
void PlayerEntity::renderHead(SDL2pp::Renderer &renderer, Camera &camera) {
    ...
    Sprite src = textureManager.getHeadSprite(
        player.getHeadID(),
        player.getDirection()
    );
    ...
    renderer.Copy(src.txt,
                   SDL2pp::Rect(src.x, src.y, src.w, src.h),
                   dst);
}


int PlayerEnitity::get_head_x(Camera &camera) {
...
int PlayerEntity::get_head_y(Camera &camera) {
    ...
    SDL2pp::Rect playerPosition = camera.toScreen(player.get_x(), player.get_y(), Player::Width, Player::Height);
    //24
    return headY = playerPosition.y - Player::HeadHeight + 4;
}



int PlayerEntity::get_x()
{
    return player.get_x();
}

int PlayerEntity::get_y()
{
    return player.get_y();
}
