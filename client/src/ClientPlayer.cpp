...
void ClientPlayer::setInventory(
    const std::array<uint8_t, MAX_INVENTORY_SLOTS> &items) {
  player.setInventoryItems(items);
}
void ClientPlayer::setEquippedWeapon(uint8_t id) {
  player.setEquippedWeapon(id);
}
void ClientPlayer::setEquippedArmor(uint8_t id) { player.setEquippedArmor(id); }
void ClientPlayer::setEquippedHelmet(uint8_t id) {
  player.setEquippedHelmet(id);
}
void ClientPlayer::setEquippedShield(uint8_t id) {
  player.setEquippedShield(id);
}
...
void ClientPlayer::setBeingAttacked(bool v, EffectType effect) {
  beingAttacked = v;
  if (v)
    currentEffect = effect;
}
void ClientPlayer::stopAttackEffect() const {
  beingAttacked = false;
  currentEffect = EffectType::NormalAttack;
}
uint8_t ClientPlayer::getEquippedWeapon() const {
  return player.getEquippedWeapon().getID();
}
uint8_t ClientPlayer::getEquippedArmor() const {
  return player.getEquippedArmor().getID();
}
uint8_t ClientPlayer::getEquippedHelmet() const {
  return player.getEquippedHelmet().getID();
}
uint8_t ClientPlayer::getEquippedShield() const {
  return player.getEquippedShield().getID();
}
const Inventory &ClientPlayer::getInventory() const {
