...
void PlayerListEventParser::serialize(std::vector<uint8_t> &bytes,
  ...
  for (const auto &player : event.players) {
    ...
    utils.appendBytes(static_cast<uint8_t>(player.playerClass), bytes);
...
ServerEventDTO PlayerListEventParser::deserialize(Protocol &protocol) {
  ...
  for (uint16_t i = 0; i < playerCount; ++i) {
    ...
    PlayerClass playerClass = static_cast<PlayerClass>(protocol.getUint8());
    ...
    players.push_back(PlayerInfoDTO{
        playerId, x, y, direction, race, playerClass, std::move(playerName), hp,
        maxHp, mana, maxMana, gold, level, experience});
  }
