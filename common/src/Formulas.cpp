...
double Formulas::getFRazaVida(const std::string &raza) {
  ...
  if (lr == "elfo")   return 0.8;
  if (lr == "enano")  return 1.3;
  if (lr == "gnomo")  return 1.1;
  return 1.0;
}

double Formulas::getFRazaMana(const std::string &raza) {
  std::string lr = toLower(raza);
  if (lr == "elfo")   return 1.3;
  if (lr == "enano")  return 0.7;
  if (lr == "gnomo")  return 1.2;
  return 1.0;
}

double Formulas::getFRazaRecuperacion(const std::string &raza) {
  std::string lr = toLower(raza);
  if (lr == "elfo")   return 1.2;
  if (lr == "enano")  return 0.8;
  if (lr == "gnomo")  return 0.9;
  return 1.0;
}

double Formulas::getFClaseVida(const std::string &clase) {
  std::string lc = toLower(clase);
  if (lc == "guerrero") return 1.4;
  if (lc == "mago")     return 0.7;
  if (lc == "clerigo")  return 1.0;
  if (lc == "paladin")  return 1.2;
  return 1.0;
}

double Formulas::getFClaseMana(const std::string &clase) {
  if (isGuerrero(clase)) return 0.0;
  std::string lc = toLower(clase);
  if (lc == "mago")     return 1.5;
  if (lc == "clerigo")  return 1.2;
  if (lc == "paladin")  return 0.8;
  return 1.0;
}
...
uint32_t Formulas::calcularRecuperacionVida(const std::string &raza,
                                             uint32_t segundos) {
  double resultado = getFRazaRecuperacion(raza) *
...
uint32_t Formulas::calcularOroExceso(uint32_t oroActual, uint32_t nivel) {
  ...
  if (oroActual <= oroMax) return 0;
  if (oroActual > tope) return tope - oroMax;
...
uint32_t Formulas::calcularExperiencia(uint32_t daño, uint32_t nivelAtacante,
  ...
  int diff = static_cast<int>(nivelVictima) - static_cast<int>(nivelAtacante) + 10;
  if (diff < 0) diff = 0;
...
uint32_t Formulas::calcularExperienciaMuerte(uint32_t vidaMax,
  ...
  int diff = static_cast<int>(nivelVictima) - static_cast<int>(nivelAtacante) + 10;
  if (diff < 0) diff = 0;
  double resultado = factor * static_cast<double>(vidaMax) *
...
uint32_t Formulas::calcularOroDropNPC(uint32_t vidaMaxNPC, double randomFactor) {
  double r = clamp01(randomFactor) * 0.2;
  return static_cast<uint32_t>(r * static_cast<double>(vidaMaxNPC));
}

uint32_t Formulas::calcularOroPerdidoMuerte(uint32_t oroActual, uint32_t nivel) {
