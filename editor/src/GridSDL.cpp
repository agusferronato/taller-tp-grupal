...
void GridSDL::saveMap(const std::string &path) { grid->saveMap(*this, path); }
...
void GridSDL::updateFloatingButtonsPos() {
  if (grid->getSelectedTile()) {
    ...
    SDL2pp::Rect screenPos =
        camera->toScreen((max_i + 1 - MAX_SIZE / 2) * GRID_SIZE_PX,
    ...
    SDL2pp::Rect screenPos =
        camera->toScreen((biome.i_end + 1 - MAX_SIZE / 2) * GRID_SIZE_PX,
