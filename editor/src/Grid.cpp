...
Grid::Grid(Camera &camera, SDL2pp::Renderer &renderer, int max_priority)
    : tilesToRender(std::vector<
                    std::multimap<std::pair<int, int>, std::shared_ptr<Tile>>>(
          max_priority)),
...
void Grid::loadMap(const std::string &mapPath, TextureMap &textureMap) {
  ...
  for (auto &origin : txtOrigins) {
    auto tile =
        createTileInstance(textureMap, origin.texture_id, origin.x, origin.y);
...
void Grid::renderCommonGround(SDL2pp::Renderer &renderer) {
  ...
  int startX = (-MAX_SIZE / 2) * GRID_SIZE_PX;
  int startY = (-MAX_SIZE / 2) * GRID_SIZE_PX;
  int totalWidth = MAX_SIZE * GRID_SIZE_PX;
  int totalHeight = MAX_SIZE * GRID_SIZE_PX;
...
void Grid::setInitBiomePosition(Biome biome) {
  ...
  biomes[currentBiomePlacingId] = BiomeGrid{currentBiomePlacingId,
                                            biome,
...
void Grid::updateSelectedBiome() {
  ...
  biomes[currentBiomePlacingId].i_init =
      std::min(biomes[currentBiomePlacingId].i_start, item_hover_i);
  biomes[currentBiomePlacingId].i_end =
      std::max(biomes[currentBiomePlacingId].i_start, item_hover_i);
  biomes[currentBiomePlacingId].j_init =
      std::min(biomes[currentBiomePlacingId].j_start, item_hover_j);
  biomes[currentBiomePlacingId].j_end =
      std::max(biomes[currentBiomePlacingId].j_start, item_hover_j);
}
...
std::shared_ptr<Tile> Grid::createTileInstance(TextureMap &textureMap,
  ...
  auto tile = std::make_shared<Tile>(next_instance_id++, texture_id,
                                     txtInMap.data.priority, start_i, start_j,
  ...
  for (int j = start_j; j < start_j + rows; j++) {
    ...
    for (int i = start_i; i < start_i + columns; i++) {
      ...
      SDL2pp::Rect rect = {item.x_start, item.y_start,
                           item.x_end - item.x_start,
                           item.y_end - item.y_start};
      if (j >= start_j + offset &&
...
std::map<int, BiomeGrid> &Grid::getBiomes() { return biomes; }
...
void Grid::setHoverTexture(TextureMap &textureMap, int texture_id) {
  ...
  hoverTile =
      createTileInstance(textureMap, texture_id, item_hover_i, item_hover_j);
}
...
bool Grid::checkCollisions(const std::shared_ptr<Tile> &tile) {
  ...
  std::multimap<std::pair<int, int>, std::shared_ptr<Tile>> &tiles =
      tilesToRender[tile->getPriority()];

  for (const auto &[_, placedTile] : tiles) {

    if (placedTile->getId() == tile->getId())
      continue;

    for (const auto &cell : tile->getCollidableCells()) {

      for (const auto &placedCell : placedTile->getCollidableCells()) {
        if (std::get<0>(cell) == std::get<0>(placedCell) &&
...
void Grid::tryPlaceHoverTexture(TextureMap &textureMap) {
  if (!hoverTile || checkCollisions(hoverTile))
    return;

  txtOrigins.push_back({hoverTile->getPriority(), hoverTile->getTextureId(),
                        hoverTile->getOriginI(), hoverTile->getOriginY()});

  for (auto cell : hoverTile->getCollidableCells()) {
    collidableCells.insert(cell);
  }

  hoverTile = createTileInstance(textureMap, active_texture_id, item_hover_i,
                                 item_hover_j);
}
...
bool Grid::selectElementAt(int i, int j) {
  ...
  for (auto const &[id, biome] : biomes) {
    if (biome.initialized && i >= biome.i_init && i <= biome.i_end &&
        j >= biome.j_init && j <= biome.j_end) {
      selectedBiomeId = id;
      return true;
    }
  }
...
void Grid::renderHoverAndSelection(SDL2pp::Renderer &renderer) {
  ...
  if (selectedTile) {
    ...
    SDL2pp::Rect dstRect = camera.toScreen(
        ...
        (max_i - min_i + 1) * GRID_SIZE_PX, (max_j - min_j + 1) * GRID_SIZE_PX);
    ...
    SDL2pp::Rect inner = {dstRect.x + 1, dstRect.y + 1, dstRect.w - 2,
                          dstRect.h - 2};
