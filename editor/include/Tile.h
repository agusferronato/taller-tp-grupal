...
public:
  Tile(int id, int texture_id, int priority, int start_i, int start_j, int w_px,
       int h_px)
      : id(id), texture_id(texture_id), priority(priority), origin_i(start_i),
        origin_j(start_j), width_px(w_px), height_px(h_px) {}
  ...
  const std::set<std::pair<int, int>> &getCollidableCells() const {
    return localCollidables;
  }
  ...
  void updatePosition(int new_i, int new_j) {
    ...
    for (auto &cell : localCollidables) {
      updatedCollidables.insert(
          {cell.first + offset_i, cell.second + offset_j});
    }
