...
NPC::NPC(Position pos, const NPCStats &stats)
    : gridPosition(pos), range(stats.range),
      attackCounterMax(stats.attackCounterMax), x(0), y(0), hp(stats.hp),
      maxHp(stats.hp), level(stats.level), agility(stats.agility),
      damage(stats.damage), ancho(stats.ancho), alto(stats.alto),
      type(stats.type), name(stats.name) {}

bool NPC::colisionaCon(int targetX, int targetY, int targetAncho,
                       int targetAlto) const {
  return !(targetX + targetAncho <= x || targetX >= x + getAncho() ||
           targetY + targetAlto <= y || targetY >= y + getAlto());
}
...
bool NPC::collidesWith(Character &character) {
  return colisionaCon(character.getX(), character.getY(), character.getAncho(),
                      character.getAlto());
}
...
void NPC::setPixelPosition(int px, int py) {
  x = px;
  y = py;
}
