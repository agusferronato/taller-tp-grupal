...
NPCData::NPCData(const std::string &path) {
  ...
  if (auto *npcTable = tbl["npc"].as_table()) {
    for (auto &[key, value] : *npcTable) {
      ...
      npcs[static_cast<uint8_t>(std::stoi(std::string(key)))] =
          std::move(npcInfo);
    }
  }
}
...
int NPCData::getWidth(uint8_t npcId) const { return npcs.at(npcId).width; }

int NPCData::getHeight(uint8_t npcId) const { return npcs.at(npcId).height; }
